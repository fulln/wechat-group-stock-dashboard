...
the one-click script fetches Google Finance snapshots, intraday charts, and
speaker-page daily K data unless disabled. Use `--no-market-data` only when the
user asks for a lightweight/offline run or explicitly does not want external
market fetches. The speaker page is still generated, but daily K is disabled.
...
Market snapshots and speaker-page daily K are included in deployment commands by default. Add `--no-market-data` only when external market fetches should be skipped.
